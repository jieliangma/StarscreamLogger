...
public class FoundationHTTPHandler: HTTPHandler {
    ...
    func parseContent(data: Data) -> Bool {
        ...
        if let cfHeaders = CFHTTPMessageCopyAllHeaderFields(response), let command = String(data: data, encoding: .utf8)?.components(separatedBy: "\r\n").first {
            ...
            guard components.count >= 2, let url = URL(string: components[1]) else {
                delegate?.didReceiveHTTP(event: .failure(HTTPUpgradeError.invalidData))
                return true
            }
            
            let request = NSMutableURLRequest(url: url)
            request.httpMethod = components[0]
            request.allHTTPHeaderFields = headers
            
            delegate?.didReceiveHTTP(event: .success(request as URLRequest))
            return true
        }
